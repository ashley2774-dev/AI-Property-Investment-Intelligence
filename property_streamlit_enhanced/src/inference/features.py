...
def build_features(payload: dict) -> tuple[dict, dict]:
    ...
    bond_to_rent = financials["monthly_bond_payment"] / monthly_rent if monthly_rent > 0 else 0.0
    opex_to_rent = ((financials["opex_annual"] / 12) / monthly_rent) if monthly_rent > 0 else 0.0
